...
//函数定义：
/**
  * @brief  LCD1602延时函数，12MHz调用可延时1ms,可改。
  * @param  无
  * @retval 无
  */
void LCD_Delay()
...
/**
  ...
  * @retval 无
  */
void LCD_ShowHexNum(uchar Line,uchar Column,uint Number,uchar Length)
{
	uchar i,SingleNumber;
	LCD_SetCursor(Line,Column);
	for(i=Length;i>0;i--)
	{
...
/**
  ...
  * @retval 无
  */
void LCD_ShowBinNum(uchar Line,uchar Column,uint Number,uchar Length)
{
	uchar i;
	LCD_SetCursor(Line,Column);
	for(i=Length;i>0;i--)
